...
const AddPro = () => {
  ...
  const fetchSubCategories = (categoryId) => {
    axios
      .get(`${process.env.REACT_APP_URL}/subcategory/getByCategory?category=${categoryId}`)
      .then((response) => {
        setSubCategories(response.data.data);
      })
      .catch((error) => {
      ...
      <form onSubmit={handleSubmit}>
                  ...
                  <div>
                    ...
                    {char.options.map((opt, optIndex) => (
                      <div key={optIndex} className="input-group mb-2">
                        ...
                        <button
                          type="button"
                          className="btn btn-danger" style={{ backgroundColor: 'red', borderColor: 'red' }}
                          onClick={() =>
                            handleRemoveOption(charIndex, optIndex)
                          }
                        >
        ...
        <div className="d-flex justify-content-between">
          ...
          <button
            type="button"
            className="btn btn-secondary"
            onClick={handleCancel} tyle={{ backgroundColor: 'gray', borderColor: 'gray' }}
          >
            Cancel
          </button>
        </div>
